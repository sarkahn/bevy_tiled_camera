...
impl SizedGrid {
    ...
    /// 
    /// Returns None if the position is out of bounds.
    pub fn tile_to_tile_center_world(&self, transform: &GlobalTransform, tile_pos: (i32,i32)) -> Option<Vec3> {
        let tile_pos = IVec2::from(tile_pos);
        if !self.tile_in_bounds(tile_pos) {
            return None;
        }
    ...
    pub fn tile_in_bounds(&self, tile_pos: IVec2) -> bool {
        let (min, max) = match self.centered {
            ...
            false => {
                (IVec2::ZERO, self.tile_count.as_ivec2())
            },
        };
...
#[cfg(test)]
mod test {
    use bevy::{math::{IVec2, Vec2, Vec3}, prelude::GlobalTransform};
